 ...
 */
diff_match_patch.prototype.main = function(text1, text2, opt_checklines,
    opt_deadline) {
 ...
 * @param {!Array.<!diff_match_patch.Diff>} diffs Array of diff tuples.
 */
diff_match_patch.prototype.cleanupSemantic = function(diffs) {
  var changes = false;
 ...
 * @param {!Array.<!diff_match_patch.Diff>} diffs Array of diff tuples.
 */
diff_match_patch.prototype.cleanupEfficiency = function(diffs) {
  var changes = false;
  var equalities = [];  // Stack of indices where equalities are found.
  var equalitiesLength = 0;  // Keeping our own length var is faster in JS.
  /** @type {?string} */
 ...
 */
diff_match_patch.prototype.html = function(diffs) {
  var html = [];
  var pattern_amp = /&/g;
  var pattern_lt = /</g;
  var pattern_gt = />/g;
  var pattern_para = /\n/g;
  var no_break_space = /&nbsp;/g;
  for (var x = 0; x < diffs.length; x++) {
...
diff_match_patch.prototype.patch_make = function(a, opt_b, opt_c) {
  ...
  if (typeof a == 'string' && typeof opt_b == 'string' &&
    ...
    text1 = /** @type {string} */(a);
    diffs = this.main(text1, /** @type {string} */(opt_b), true);
    if (diffs.length > 2) {
      this.cleanupSemantic(diffs);
      this.cleanupEfficiency(diffs);
    }
  ...
  for (var x = 0; x < patches.length; x++) {
    ...
    if (start_loc == -1) {
      ...
      if (text1 == text2) {
        ...
        // indices.
        var diffs = this.main(text1, text2, false);
        if (text1.length > this.Match_MaxBits &&
